...
def generate_jpgs_and_csv(path_to_csv, set_size=1000, start_set=None, end_set=None, subject_set_name='SubjectSet',
    ...
    for number_list, group in enumerate(spiral_multilist):
        ...
        for n, (image_id, ra, dec) in enumerate(group):
            ...
            size = 512
            # DECAL FITS
            url = "http://legacysurvey.org/viewer/fits-cutout?ra="+str(ra_degree)+"&dec="+str(dec_degree)+"&size=512&layer=decals-dr5&pixscale=0.227&bands=g"
            image_file = folder + str(image_id) + "D.fits"
            jpg = urllib.urlretrieve(url, image_file)
            if(os.path.getsize(image_file) > 3000):
                ...
                if(a == 0.0 or b == 0.0 or c == 0.0 or d == 0.0 or e == 0.0):
                    ...
                    url = "http://legacysurvey.org/viewer/fits-cutout?ra="+str(ra_degree)+"&dec="+str(dec_degree)+"&size=512&layer=sdss2&pixscale=0.227&bands=g"
                    image_file = folder + str(image_id) + "S.fits"
                    jpg = urllib.urlretrieve(url, image_file)
                    image = fits.open(image_file)
                    data = image[0].data
                    ...
                    vmax = max(data[int(size/2),int(size/2)],
                               ...
                               data[int(size/2)-1,int(size/2)+1])
                    image.close()
                    jpeg_name = folder + str(image_id) + "S.jpeg"
                    os.system("ds9 " +image_file+" -scale limits " +str(vmin)+" "+str(vmax)+ " -log -export jpeg "+jpeg_name+" 100 -quit")
                #os.remove(image_file)
                    ...
                    vmax = max(data[int(size/2),int(size/2)],
                               ...
                               data[int(size/2)+1,int(size/2)-1],
                               data[int(size/2)-1,int(size/2)+1])
                    jpeg_name = folder + str(image_id) + "D.jpeg"
                    os.system("ds9 " +image_file+" -scale limits " +str(vmin)+" "+str(vmax)+ " -log -export jpeg "+jpeg_name+" 100 -quit")
            else:
                # SDSS FITS
                total = total + 1
            ...
            writer.writerow([n, jpeg_name, url, info_url, leda_url, ra_degree, dec_degree])
                #image_file = folder + str(image_id) + "B.fits"
                #os.remove(image_file)
